...
void IsvalytiString(string& eil) {
	std::transform(eil.begin(), eil.end(), eil.begin(), ::tolower);Didinimas;
	std::regex reg("\\W");
...
MAPw Skaitymas(Klase_& klase, int numeris) {
	...
	for (uint i = 0; !in.eof(); ++i) {
		std::getline(in, eil, '\n');                                                   //skaitom eilute iki jos galo
		if (klase.TikrinimasTiesa()) PasalintiURLS(eil);                                   //jeigu pasirinkta kad pasalintumem ulr, juos salinam.
		IsvalytiString(eil);                                                                //sutvarkom didziasias ir mazasias raides

		std::stringstream ss(eil);
		string zodis;
		while (ss >> zodis) {
			if (!ArSarase(Zodziai, zodis)) {                                          //jeigu neranda tokio zodzio ji iraso
				vector<uint> init{ 1 };
